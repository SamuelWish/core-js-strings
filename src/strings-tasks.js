...
function removeLastOccurrences(str, value) {
  const lastTime = str.lastIndexOf(value);
  if (lastTime === -1) return str;
  return str.slice(0, lastTime) + str.slice(lastTime + value.length);
}
...
function isPalindrome(str) {
  const smallAndClean = str.toLowerCase().replace(/[\W_]/g, '');
  const otherwise = smallAndClean.split('').reverse().join('');
  return otherwise === smallAndClean;
}
...
function findLongestWord(sentence) {
  ...
  const longestWord = sentenceSplit.sort(function notBigger(word1, word2) {
